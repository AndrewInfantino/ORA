...
/* ---- 7. Waitlist form — FormSubmit (zero config, no account needed) ---- */
...
form.addEventListener('submit', async (e) => {
  ...
  formData.append('message', 'New ORA waitlist signup — Name: ' + name + ' | Email: ' + email);
  formData.append('_subject', 'New ORA Waitlist Signup — ' + name);
  formData.append('_captcha', 'false');
  formData.append('_template', 'table');

  try {
    const res = await fetch('https://formsubmit.co/ajax/andrewinfantino@gmail.com', {
      method: 'POST',
